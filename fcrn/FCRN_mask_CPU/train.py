...
def main():
    ...
    print("Loading data......")
    train_dataset = GelsightImagesDataset('train','contact')
    validation_dataset = GelsightImagesDataset('validation','contact')
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for gelsight_image, gt_image, _ in validation_data_loader:
                ...
                input_gt_image = input_gt_image[0].data.cpu().numpy().astype(np.uint8)
                pred_image = output[0].data.squeeze().cpu().numpy().astype(np.uint8)                            
                                
                plot.imsave('train_output/input_rgb_epoch_{}.png'.format(start_epoch + epoch + 1), input_rgb_image)
                plot.imsave('train_output/gt_image_epoch_{}.png'.format(start_epoch + epoch + 1), input_gt_image, cmap="gray")
                plot.imsave('train_output/pred_image_epoch_{}.png'.format(start_epoch + epoch + 1), pred_image, cmap="gray")
