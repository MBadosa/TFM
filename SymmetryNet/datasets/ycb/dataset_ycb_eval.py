...
class SymDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        model_points = np.delete(self.cld[obj[idx]], dellist, axis=0)  # (500,3)

        target = np.dot(model_points, target_r.T)  # (500,3)
        # target_s = np.add(np.dot(model_s, target_r.T), target_t)
        ...
        np.save('cloud.npy',cloud.astype(np.float32))
        np.save('choose.npy', choose.astype(np.int32))
        np.save('img_masked.npy', img_masked.astype(np.float32))
        print("imageeee",torch.from_numpy(img_masked.astype(np.float32)).shape)
